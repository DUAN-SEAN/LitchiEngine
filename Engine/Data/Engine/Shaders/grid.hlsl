...
PixelInput mainVS(Vertex_PosUvNorTan input)
{
    ...
    output.position       = mul(output.position_world, buffer_rendererPath.view_projection_unjittered);
...
float4 mainPS(PixelInput input) : SV_TARGET
{
    ...
    float camera_dist = length(buffer_rendererPath.camera_position.xz - world_pos.xz);
