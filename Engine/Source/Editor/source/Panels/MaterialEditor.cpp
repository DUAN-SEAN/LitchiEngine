...
void DrawHybridVec3(WidgetContainer& p_root, const std::string& p_name, glm::vec3& p_data, float p_step, float p_min, float p_max)
{
...
void DrawHybridVec4(WidgetContainer& p_root, const std::string& p_name, glm::vec4& p_data, float p_step, float p_min, float p_max)
{
	GUIDrawer::CreateTitle(p_root, p_name);
...
void LitchiEditor::MaterialEditor::SetTarget(Resource::Material & p_newTarget)
{
...
Resource::Material * LitchiEditor::MaterialEditor::GetTarget() const
{
	return m_target;
}
...
void LitchiEditor::MaterialEditor::GenerateShaderSettingsContent()
{
	...
	for (auto& [order, info] : sortedUniformsData)
	{
		...
		if (uniformData)
		{
			...
			case UniformType::UNIFORM_FLOAT_VEC2:	GUIDrawer::DrawVec2(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<glm::vec2&>(*info.second), 0.01f, GUIDrawer::_MIN_FLOAT, GUIDrawer::_MAX_FLOAT);	break;
			case UniformType::UNIFORM_FLOAT_VEC3:	DrawHybridVec3(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<glm::vec3&>(*info.second), 0.01f, GUIDrawer::_MIN_FLOAT, GUIDrawer::_MAX_FLOAT);			break;
			case UniformType::UNIFORM_FLOAT_VEC4:	DrawHybridVec4(*m_shaderSettingsColumns, UniformFormat(info.first), reinterpret_cast<glm::vec4&>(*info.second), 0.01f, GUIDrawer::_MIN_FLOAT, GUIDrawer::_MAX_FLOAT);			break;
