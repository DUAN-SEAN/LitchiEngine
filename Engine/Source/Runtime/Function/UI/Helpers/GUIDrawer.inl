...
#include "Runtime/Function/UI/Widgets/Drags/DragSingleScalar.h"
...
namespace LitchiRuntime
{
	...
	template<typename T>
	inline void GUIDrawer::DrawScalar(WidgetContainer & p_root, const std::string & p_name, T & p_data, float p_step, T p_min, T p_max, Event<>* p_updateNotifier /*= nullptr*/)
	{
	...
	template<typename T>
	inline void GUIDrawer::DrawScalar(WidgetContainer & p_root, const std::string & p_name, std::function<T(void)> p_gatherer, std::function<void(T)> p_provider, float p_step, T p_min, T p_max, Event<>* p_updateNotifier /*= nullptr*/)
	{
		static_assert(std::is_scalar<T>::value, "T must be a scalar");
