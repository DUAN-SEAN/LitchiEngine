...
namespace Spartan
{
    ...
    void Window::Initialize()
    {
        ...
        if (SDL_WasInit(SDL_INIT_VIDEO) != 1)
        {
            ...
            {
                DEBUG_LOG_ERROR("Failed to initialise SDL video subsystem: %s.", SDL_GetError());
                return;
            }
        }
        ...
        if (!window)
        {
            DEBUG_LOG_ERROR("Could not create window: %s.", SDL_GetError());
            return;
        }
            ...
            if (sdl_event.type == SDL_WINDOWEVENT)
            {
                if (sdl_event.window.windowID == SDL_GetWindowID(window))
                {
                    ...
                    case SDL_WINDOWEVENT_ICCPROF_CHANGED:
                        DEBUG_LOG_INFO("The ICC profile of the window's display has changed");
                        break;
                    case SDL_WINDOWEVENT_DISPLAY_CHANGED:
                        DEBUG_LOG_INFO("Display has been changed, detecting new display modes");
                        Display::DetectDisplayModes();
                        break;
                    default:
                        DEBUG_LOG_ERROR("Unhandled window event");
                        break;
                    }
                }
            }
